...
def getweek(s, game=False):
    '''get the week from a well-formatted file name or game.'''
    if game:
        re_str = r"2017\.week(\d+)\.[A-Za-z_49]+@[A-Za-z_49]+\.csv"
    else:
        re_str = r"2017\.week(\d+)\.htm"
    return int(re.match(re_str, s).group(1))
